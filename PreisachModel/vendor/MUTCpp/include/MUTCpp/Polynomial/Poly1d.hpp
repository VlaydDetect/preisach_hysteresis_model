...
namespace mc
{
    namespace polynomial
    {
        ...
        public:
            ...
            /// @param isRoots
            Poly1d(const Matrix<dtype> &inValues, IsRoots isRoots = IsRoots::NO)
            {
                ...
                if (isRoots == IsRoots::YES)
                {
                    ...
                    {
                        Matrix<dtype> coeffs = {-(value), static_cast<dtype>(1)};
                ...
                else
                {
                    ...
                    algo::copy(inValues.begin(), inValues.end(), coefficients_.begin());
                }
            }
            ...
            {
                auto newCoefficients = Matrix<dtypeOut>(1, static_cast<uint32>(coefficients_.size()));

                const auto function = [](dtype value) -> dtypeOut { return static_cast<dtypeOut>(value); };

                algo::transform(coefficients_.begin(), coefficients_.end(), newCoefficients.begin(),
                                function);
            ...
            /// @return Matrix
            ///
            [[nodiscard]] Matrix<dtype> coefficients() const
            {
                auto coefficientsCopy = coefficients_;
                return Matrix<dtype>(coefficientsCopy);
            }
            ...
            [[nodiscard]] Poly1d<dtype> deriv() const
            {
                ...
                Matrix<dtype> derivativeCofficients(1, numCoefficients - 1);
            ...
            ///
            Matrix<dtype> eval(const Matrix<dtype> &xValues) const noexcept
            {
            ...
            /// @return Poly1d
            static Poly1d<double> fit(const Matrix<dtype> &xValues, const Matrix<dtype> &yValues, uint8 polyOrder)
            {
                ...
                Matrix<double> a(numMeasurements, polyOrder + 1);
            ...
            /// @param polyOrder: the order of the poly nomial to fit
            /// @return Poly1d
            static Poly1d<double> fit(const Matrix<dtype> &xValues,
                                      const Matrix<dtype> &yValues,
                                      const Matrix<dtype> &weights,
                                      uint8 polyOrder)
            {
                const auto numMeasurements = xValues.size();
                ...
                Matrix<double> aWeighted(a.shape());
                Matrix<double> yWeighted(yValues.shape());
            ...
            [[nodiscard]] Poly1d<double> integ() const
            {
                ...
                Matrix<double> integralCofficients(1, numCoefficients + 1);
            ...
            /// @return predicted yValues
            ///
            Matrix<dtype> operator()(const Matrix<dtype> &xValues) const noexcept
            {
                Matrix<dtype> returnArray(xValues.shape());

                algo::transform(xValues.begin(),
                                xValues.end(),
                                returnArray.begin(),
            ...
            Poly1d<dtype> &operator*=(const Poly1d<dtype> &inOtherPoly)
            {
                ...
                algo::copy(coefficients_.begin(), coefficients_.end(), coeffsA.begin());
                algo::copy(inOtherPoly.coefficients_.cbegin(), inOtherPoly.coefficients_.cend(),
                           coeffsB.begin());
