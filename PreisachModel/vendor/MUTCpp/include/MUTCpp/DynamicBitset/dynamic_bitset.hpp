...
namespace mc
{
    ...
    class dynamic_bitset
    {
        ...
        static_assert(static_cast<bool>(detail::dynamic_bitset_impl::allowed_block_type<Block>::value));
    ...
    public:
        ...
        private:
            ...
            void do_set() const { m_block |= m_mask; }
            void do_reset() const { m_block &= ~m_mask; }
            void do_flip() const { m_block ^= m_mask; }
            void do_assign(bool x) const { x ? do_set() : do_reset(); }
        };
    ...
    private:
        ...
        static Block set_block_bits(Block block, size_type first,
                                    size_type last, bool val) noexcept
        {
            if (val)
        ...
        inline static Block set_block_full(Block) noexcept
        {
        ...
        inline static Block reset_block_full(Block) noexcept
        {
            return 0;
        }
                              ...
                              size_type num_bits)
        {
            ...
            typedef std::basic_string<CharT, Traits, Alloc> StrT;
                                 ...
                                 const Allocator& alloc*/)
        {
            ...
            {
                const num_type mask = (static_cast<num_type>(1) << num_bits) - 1;
                value &= mask;
            }
    ...
    swap(dynamic_bitset<Block, Allocator> &b) noexcept
    // no throw
    {
    ...
    template <typename Block, typename Allocator>
    inline dynamic_bitset<Block, Allocator>::
    dynamic_bitset(dynamic_bitset<Block, Allocator> &&b) noexcept :
    ...
    operator=(dynamic_bitset<Block, Allocator> &&b) noexcept
    {
        if (addressof(b) == this) { return *this; }
    ...
    {
        if (pos >= m_num_bits)
            THROW_OUT_OF_RANGE_ERROR("mc::dynamic_bitset::at out_of_range");
    ...
    dynamic_bitset<Block, Allocator>::count() const noexcept
    {
        ...
        enum
        {
            mode = (no_padding && enough_table_width && uneffective_popcount)
            ? access_by_bytes
            : access_by_blocks
        };

        return do_count(m_bits.begin(), num_blocks(), Block(0),
                        static_cast<value_to_type<(bool)mode> *>(nullptr));
    }
    ...
    to_ulong() const
    {
        ...
        if (find_next(ulong_width - 1) != npos)
            THROW_OVERFLOW_ERROR("mc::dynamic_bitset::to_ulong overflow");
    ...
    dynamic_bitset<Block, Allocator>::max_size() const noexcept
    {
        ...
        return m <= (static_cast<size_type>(-1) / bits_per_block) ? m * bits_per_block : static_cast<size_type>(-1);
    }
        ...
        if (cerberos)
        {
...

            {
                try
                {
                    ...
                    // if needed fill at a left; pad is decreased along the way
...
{
    template <typename Block, typename Allocator>
    struct hash<mc::dynamic_bitset<Block, Allocator>>
    {
        typedef mc::dynamic_bitset<Block, Allocator> argument_type;
        typedef std::size_t result_type;

        result_type operator()(const argument_type &a) const noexcept
        {
            hash<argument_type> hasher;
            return hasher(a);
        }
    };
}
