...
#define NAIVE_MONTE_CARLO_DEBUG_FAILURES true
...
#ifdef NAIVE_MONTE_CARLO_DEBUG_FAILURES
#include <iostream>
#endif
...
namespace mc
{
    namespace integrate
    {
        namespace detail
        {
            enum class limit_classification
            {
                FINITE,
        ...
        template <class Real, class F, class RandomNumberGenerator = std::mt19937_64,
                  std::enable_if_t<std::is_trivially_copyable_v<Real>, bool>  = true>
        ...
        public:
            naive_monte_carlo(const F &integrand,
                              ...
                              Real error_goal,
                              bool singular = true,
                              uint64_t threads = std::thread::hardware_concurrency() - 1,
                              uint64_t seed = 0) noexcept :
                m_num_threads{threads}, m_seed{seed}, m_volume(1)
            {
                ...
                static const char *function = "mc::integrate::naive_monte_carlo<%1%>";
                for (uint64_t i = 0; i < n; ++i)
                {
                    if (bounds[i].second <= bounds[i].first)
                    {
                        THROW_RUNTIME_ERROR(std::format("{}: {}: {}", function, "The upper bound is <= the lower bound.\n", bounds[i].second));
                        return;
                    }
                    if (isinf(bounds[i].first))
                    {
                        ...
                        else
                        {
                            ...
                            // Ok this is bad to use the second bound as the lower limit and then reflect.
        ...
        private:
                ...
                do
                {
                    ...
                    do
                    {
                        ...
                        for (uint64_t i = 0; i < m_num_threads; ++i)
                        {
                            ...
                            avg += m_thread_averages[i].load(std::memory_order_relaxed) * (static_cast<Real>(t_calls) /
                                static_cast<Real>(total_calls));
                    ...
                    while (m_total_calls < 2048 || this->current_error_estimate() > m_error_goal.load(
            ...
            void m_thread_monte(uint64_t thread_index, uint64_t seed)
            {
                    ...
                    while (!m_done) // relaxed load
                    {
                        ...
                        while (j++ < magic_calls_before_update)
                        {
                            ...
                            if (!isfinite(f))
                            {
                                ...
                                static const char *function = "quadrature::naive_monte_carlo<%1%>";
                                THROW_RUNTIME_ERROR(std::format("{}: {}: {}", function, os.str().c_str(), 7.2));
                            }
                            ++k;
                            Real term = (f - M1) / k;
                ...
                catch (...)
                {
                    ...
                    // Scoped lock to prevent race writing to m_exception
