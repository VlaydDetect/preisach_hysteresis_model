...
namespace mc
{
    namespace edac
    {
        namespace detail
        {
            ...
            /// Calculates the number of necessary Hamming SECDED parity bits to encode the data
            ///
            ...
            /// Calculates the overall parity of the data, assumes the last bit is the parity bit itself
            ///
            ...
            /// @return overall parity bit value
            ///
            inline bool calculateParity(const dynamic_bitset<> &data) noexcept
            {
                bool parity = false;
            ...
            bool calculateParity(const std::bitset<DataBits> &data, IntType parityBit)
            {
                ...
                return std::accumulate(bitsCovered.begin(),
                                       bitsCovered.end(),
                                       false,
                                       [](bool parity, const auto value) noexcept -> bool
                                       {
                                           return parity ^= value;
                                       });
            }
            ...
            /// DataBits and EncodedBits have been checked for consistancy already
            ///
        ...
        ///
        template <std::size_t DataBits>
        dynamic_bitset<> encode(const std::bitset<DataBits> &dataBits)
        {
            const auto numParityBits = detail::numSecdedParityBitsNeeded(DataBits);
            const auto numEncodedBits = numParityBits + DataBits;

            auto encodedBits = dynamic_bitset<>(numEncodedBits); // NOLINT(google-readability-casting)

            // set the parity bits
            for (const auto parityBit :
                 detail::powersOfTwo(numParityBits - 1))
            // -1 because overall parity will be calculated seperately later
            {
