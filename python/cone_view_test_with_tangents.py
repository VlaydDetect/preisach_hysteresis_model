...
def pick_two_support_rays_2d(A, X, min_norm_ratio=1e-6, eps_tangent=1e-12):
    ...
    if max_norm == 0:
        ...
        return np.vstack([v/np.linalg.norm(v), perp/np.linalg.norm(perp)]), np.array([0,0]), np.pi, {"reason":"all zero"}
    ...
    if valid.size == 0:
        v = dominant_real_eigvec(A)
        perp = np.array([-v[1], v[0]])
        return np.vstack([v/np.linalg.norm(v), perp/np.linalg.norm(perp)]), np.array([0,0]), np.pi, {"reason":"no valid points"}
